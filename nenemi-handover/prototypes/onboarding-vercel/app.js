...
function screen(){
  switch(state.stage){
    case 'signup': return `${heading('Create your account','Free to start — 2 Rooms, no card. Your space, private to you.')}<div class="stack"><button class="btn" data-action="social">Continue with Apple</button><button class="btn" data-action="social">Continue with Google</button><div class="divider">or</div></div><label class="field"><span class="label">EMAIL</span><input id="email" type="email" autocomplete="email" value="${escapeHtml(state.email)}" placeholder="you@email.com"></label><label class="field"><span class="label">PASSWORD</span><input id="password" type="password" autocomplete="new-password" value="${escapeHtml(state.password)}" placeholder="9+ characters"></label>${primary('Start free →','signup',!(state.email.includes('@')&&state.password.length>=9))}<p class="support-note">Already have a space? <button class="link">Sign in</button></p><p class="tiny">FREE FOR 2 ROOMS · NO CARD TO START</p>`;
    ...
    case 'plan': return `${heading('As much space as your head needs.','Keep the doors open. Free gives you a quiet anchor. Full access removes every limit, as many rooms, notes, and threads as your day requires.')}<div class="plan free"><h3>Free</h3><ul><li>2 Rooms of your own, with 5 document downloads per Room</li><li>Calendar, voice, and your way back in — always</li><li>Saved to your device first, synced across them</li></ul></div><div class="plan"><h3>Full access <span>($10/mo)</span></h3><ul><li>Unlimited Rooms — as many as the day needs</li><li>Unlimited document downloads in every Room</li><li>Priority context sync and advanced retrieval engines</li></ul></div>${primary('Unlock full access — $10/mo','upgrade')}<button class="link" data-action="plan">Stay on free for now</button>`;
