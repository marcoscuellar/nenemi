# NENEMI — Everything Built This Session
...
**Capture Inbox** — the always-listening front door. You type or speak a stray thought at any time. NENEMI:
...
const ROUTE_PROMPT = `You are NENEMI's capture router. The user just said or typed something offhand —
...
const BRIEF_PROMPT = `You write the "Brief" for a Room in NENEMI — a memory app for someone with ADHD.
...
const CHAT_PROMPT = `You are NENEMI — a calm, warm executive-function assistant for someone with ADHD.
...
export async function chatReply(message, history = []) {
  const content = [
    ...history.map((h) => `${h.role === 'user' ? 'User' : 'NENEMI'}: ${h.text}`),
    `User: ${message}`,
  ].join('\n');
...
function Stub({ label }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: '0.2em', opacity: 0.4 }}>NENEMI</span>
 ...
 *
 * NENEMI already had strong "help me start" (Catalyst) and "help me reset"
...
- ADHD tool users are burned by fake urgency (countdown timers, guilt-based streaks) — NENEMI should never use those patterns.
