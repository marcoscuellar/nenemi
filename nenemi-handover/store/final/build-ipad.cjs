...
.def{position:absolute;inset:0;background:#050404 url('file:///home/user/nenemi/photos/texture/tex-fret.webp') right center/cover no-repeat}.def:before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 70% 60% at 50% 50%,rgba(8,6,5,.6),rgba(8,6,5,.15))}.dc{position:absolute;left:62px;right:62px;top:50%;transform:translateY(-50%);display:flex;flex-direction:column;align-items:center;text-align:center;gap:48px}.dc svg{width:92px;height:auto;filter:drop-shadow(0 12px 34px rgba(0,0,0,.75))}.dc path{fill:#f4f1ea}.d-e{font:700 24px ui-monospace,Menlo,'DejaVu Sans Mono',monospace;letter-spacing:.2em;color:#3C8692}.d-n{font:35px 'Archivo Black',sans-serif;letter-spacing:.16em;color:#9aa0a2;margin-top:-22px}.d-s{font:110px/1 'Archivo Black',sans-serif;letter-spacing:-.02em;color:#f4f1ea;margin-top:-13px}.d-t{font:500 34px/1.65 Archivo,sans-serif;color:#9aa0a2}</style></head><body>
${x.def?`<div class="def"><div class="dc">${MARK}<div class="d-e">THE NAME</div><div class="d-n">NENEMI</div><div class="d-s">neh-NEH-mee</div><div class="d-t">Nenemi comes from Nahuatl, a living Indigenous language spoken by Nahua communities in Mexico. The word means &ldquo;to walk&rdquo; or &ldquo;to go about.&rdquo; We chose it because your path doesn&rsquo;t have to be straight to keep moving.</div></div></div>`:`<div class="copy" id="cp"><h1>${x.h}</h1><p class="sub">${x.sub}</p></div>`}
...
${x.def?'':x.closer?`<div class="card c1" style="left:60px;width:500px;height:800px;background-image:url('file://${R}photos/life-dogwalk.jpg');background-position:55% 30%"></div><div class="card c2" style="left:450px;width:520px;height:760px;background-image:url('file://${R}photos/life-dinner.webp');background-position:45% 55%;z-index:3"></div>
`:`<div class="pad" id="pd"${x.photos?' style="left:196px;width:640px"':''}><img src="file://${SH}${x.shot}.png"></div>`}
</body></html>`;
(async()=>{const b=await chromium.launch();const p=await b.newPage({viewport:{width:1032,height:1376},deviceScaleFactor:2});
 ...
 for(let i=0;i<SL.length;i++){const f=S+'/frames/ipad.html';fs.writeFileSync(f,page(SL[i]));await p.goto('file://'+f);await p.evaluate(()=>document.fonts.ready);await p.waitForTimeout(400);
  await p.evaluate(()=>{const c=document.getElementById('cp');if(!c)return;const t=c.offsetTop+c.offsetHeight+48;const pd=document.getElementById('pd');if(pd)pd.style.top=t+'px';
   document.querySelectorAll('.ph0,.ph1').forEach((e,i)=>e.style.top=(t+(i?220:60))+'px');
   const c1=document.querySelector('.c1'),c2=document.querySelector('.c2');if(c1){c1.style.top=t+'px';c2.style.top=(t+220)+'px';}});
