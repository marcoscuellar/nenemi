...
**Capture Inbox** — the always-listening front door. Marcos types or speaks a stray thought at any time, from anywhere in the app (not just inside a Room). DOS:
...
## UI additions
- **Landing page = the AI chat itself.** Not a static threshold — the main screen IS the conversational capture surface. User lands, types or speaks anything, DOS responds and routes it. Rooms Hub is one tap/rail away, not the front door.
...
- ADHD tool users are burned by fake urgency (countdown timers, guilt-based streaks) — DOS should never use those patterns anywhere, including Rooms/Memory.
...
- Separate note: Marcos also shared a full spec for a spaced-repetition study-card app (FSRS-6 scheduler, exam decks, "Not yet / Got it" grading). That's a distinct product from DOS's memory/rooms feature — flagging so it doesn't get silently merged in. Worth its own conversation on whether it's a second app, a future DOS module, or unrelated.
